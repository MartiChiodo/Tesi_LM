...
def assign_order_to_workstation_policy(order, workstations) -> int:
    ...
    """
    return min(workstations, key=lambda ws: len(ws.order_queue) + len(ws.open_orders)).workstation_id
...
def design_tasks_for_ws(workstation, warehouse, arrived_orders, task_counter) -> list[Task]:
    ...
    remaining_skus = set()
    for order_id in workstation.open_orders:
        order = arrived_orders.get(order_id)
        if order is not None:
            remaining_skus.update(order.sku_remaining)
    ...
    for v in workstation.active_tasks:
        already_selected.add(v.sku_list)

    tasks = []

    while len(tasks) < workstation.podqueue_capacity - len(workstation.active_tasks) and remaining_skus:
        ...
        for pod in warehouse.pods:
            ...
            pile_on = len(set(pod.sku_ids) & remaining_skus)
            if pile_on == 0:
                continue

            distance = warehouse.manhattan_distance(pod.home_position, workstation.position)
        ...
        for order_id in workstation.open_orders:
            ...
            if order is None:
                continue
            skus_for_order = [s for s in best_pod.sku_ids if s in set(order.sku_remaining)]
            if skus_for_order:
        ...
        tasks.append(Task(
            ...
            visits=[Visit(
                ...
                order_ids=contributing_orders,
                sku_list=skus_for_visit
            )],
            priority=task_counter + len(tasks),
        ))

        already_selected.add(best_pod.pod_id)
        remaining_skus -= set(best_pod.sku_ids)
...
def get_nearest_idle_robot(pod, warehouse) -> int | None:
    ...
    for robot in warehouse.robots:
        if robot.status == RobotStatus.IDLE:
            dist = warehouse.manhattan_distance(robot.position, pod.home_position)
