...
This thesis addresses the task design problem and the assignment of orders to workstations in RMFS by comparing two decision-making approaches. The first approach relies on greedy policies, which generate fast decisions based on local information and immediate gains. The second approach is based on a mathematical optimization model that exploits global information, allowing for a greater level of lookahead in the decision-making process. In particular, the model incorporates a space-time network representation to anticipate congestion effects. Due to the combinatorial complexity of the model, a heuristic solution method is employed to produce a solution within limited computational time. 

The proposed approaches are evaluated within a discrete-event simulation framework that captures the dynamic behaviour of the warehouse system, including stochastic order arrivals and system stochasticity. Within this framework, decisions are periodically re-optimized by solving a static optimization problem which captures the snapshot of the warehouse at a given istant, while the simulation evolves over a longer time horizon to assess system performance.  While congestions among robots naturally contribute to the variability, the precise routing of robots and the dispatching decisions are not explicitly modelled in this work. The simulation provides a reproducible environment for comparing the two strategies and allows the collection of key performance indicators (KPIs), such as order flow times and the utilization of pods and workstations. 
...
In recent literature, heuristic strategies for solving RMFS related problems cover a wide ranges of techniques. These include decomposition methods, in which the global problem is split into smaller and more maneagable optimization models soved sequentially; local search strategies, such as Variable Neighbourhood Search (VNS) or Large Neighbourhood Search (LNS),
are also commonly applied, iteratively exploring the solution space by building neighbourhoods through greedy rules or by leveraging data-driven insights, even from machine learning.
These heuristics provide practical ways to handle large combinatorial optimization problems, balancing solution quality and time computation.
