...
A \texttt{frozen} dataclass that holds every parameter that does not change across replicas. It is constructed once and shared by all \texttt{run()} calls on the same \texttt{Simulator} instance.
...
\texttt{scheduled\_tasks}   & \texttt{dict[int, PriorityQueue[Task]]} & Tasks scheduled per workstation by the optimiser \\
...
\vspace{0.8em}
The typical usage pattern in \texttt{run\_simulation.py} is therefore:

\begin{lstlisting}[language=Python, basicstyle=\ttfamily\small]
config = SimulatorConfig(order_gen_config=[...], warm_up=3600)
sim    = Simulator(rng, config, warehouse_factory=lambda: Warehouse(...))

sim.run(8 * 3600)   # replica 1 -- starts from a clean state
sim.run(8 * 3600)   # replica 2 -- fully independent, zero extra code
\end{lstlisting}
